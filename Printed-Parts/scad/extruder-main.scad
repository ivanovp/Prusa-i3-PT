...
{
    // Piezo coin
    translate([35/2,0,15]) rotate([-90,0,0]) cylinder(r=10.3, h=1, $fn=120);
    // Piezo's wire
    translate([-1,0,15])rotate([0,90,0]) cylinder(d=2, h=50, $fn=120);

    // Move everthing down, to make some space for piezo coin
    translate([0, 1, 0])
    {
        //Nozzle cutout
        // Top
